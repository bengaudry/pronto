...
fn main() {
    ...
    if args.len() != 2 {
        panic!("Expected one argument (target C file).")
    }
    ...
    if is_newer {
        ...
        let path_str = object_file_path.to_str().expect("Invalid UTF-8").to_string();
    ...
    objects.push(executable_path.to_str().expect("Could not convert path into string").to_string());
